...
由于计算最大环长度不超过50的概率相对复杂，此处选择对应的补事件，计算最大环长度超过50的概率。由于图是有向图，且每一个点的出度入度均为1，图内所有的点都需要包含于且仅包含于某一个环内，且在该图内，当存在一个最大环的长度大于50时，未被包含于该环的点一定小于50个，不可能存在另一个不同的环长度大于50，因此该图内最大环长度为51与最大环长度大于50的其他情况均为互斥事件，最大环长度为52, 53 ... 100的情况同理。因此对于最大环长度大于50的概率，可以通过如下方式计算概率。
...
采用Java程序模拟完全随机1000次和上述策略1000次，结果如下。

由于随机情况下完成的概率非常低，我们计算一下挑战失败时已经进行人数的期望，设P为在打开规定数量抽屉内找到了对应的号码。

对于每一个囚犯而言，其单独随机打开抽屉成功的概率为$50\%$，如果失败则不再继续，因此对于按照次序每个人成功的概率为$P(P=x_i)=\begin{cases}50\%& i=1\\ P(P=x_i)\times 50\%& i\in [2, 100]\\ \end{cases}$，因此整体的期望为：

$$E(P)=\frac{1}{2}\times1+\frac{1}{2^{2}}\times2+\cdots+\frac{1}{2^{100}}\times100\approx2$$
...
以上的分析思路可以应用于抽屉的总数为$2n$，允许开启$n$个抽屉$(n\gt0)$，或者抽屉的总数为$2n+1$，允许开启$n$或$n+1$个抽屉的情况$(n\geqslant0)$。

首先对于总数为$2n$的情况，可以推导假设$x（n\lt x \leqslant2n）$为最大环长度：

设事件A为最大环长度不超过$n$，设事件B为最大环长度超过$n$，X表示最大环长度。

在这种情况下，最大环长度超过n时，图中剩余点的个数少于n，因此最大环长度为$n+1$与最大环长度大于$n$的其他情况仍然属于互斥事件。

对于最大环而言，随机选取$x$个点，他们组成一个环的所有可能为$C_{2n}^{x}\cdot(x-1)!$种。$(x\gt n)$

对于剩余的$(2n-x)$个点而言，他们组成环的所有可能共有$(2n-x)!$种。

$$P(X=x)=\frac{C_{2n}^{x}\cdot(x-1)!\cdot(2n-x)!}{(2n)!}=\frac{\frac{(2n)!}{x!\cdot(2n-x)!}\cdot (x-1)!\cdot(2n-x)!}{(2n)!}=\frac{1}{x}$$
...
而对于$2n+1$允许开启$n$个抽屉的情况而言，可以推导假设$x（n\lt x \leqslant2n+1）$为最大环长度时：

设事件A为最大环长度不超过$n$，设事件B为最大环长度超过$n$，X表示最大环长度。

在这种情况下，最大环长度超过$n+1$时，图中剩余点的个数少于$n$，因此最大环长度为$n+2$与最大环长度大于$n+1$的其他情况仍然属于互斥事件。

对于最大环而言，随机选取$x$个点，他们组成一个环的所有可能为$C_{2n+1}^{x}\cdot(x-1)!$种。$(x\gt n)$

对于剩余的$(2n+1-x)$个点而言，他们组成环的所有可能共有$(2n+1-x)!$种。

$$P(X=x)=\frac{C_{2n+1}^{x}\cdot(x-1)!\cdot(2n+1-x)!}{(2n+1)!}=\frac{\frac{(2n+1)!}{x!\cdot(2n+1-x)!}\cdot (x-1)!\cdot(2n+1-x)!}{(2n+1)!}=\frac{1}{x}$$

由$P(B)=P(X=n+2)+P(X=n+3)+\cdots+P(X=2n+1)$可以得到：

$P(B)=\frac{1}{n+1}+\frac{1}{n+2}+\cdots+\frac{1}{2n+1}=\underset{\alpha=n+1}{\overset{2n+1}{\Sigma}}\frac{1}{\alpha}$

我们可以利用$\underset{\alpha=1}{\overset{2n}{\Sigma}}\frac{1}{\alpha}-\underset{\alpha=1}{\overset{n}{\Sigma}}\frac{1}{\alpha}+\frac{1}{2n+1}$得到$\underset{\alpha=n+1}{\overset{2n+1}{\Sigma}}\frac{1}{\alpha}$，由于$\underset{n\rightarrow \infty}{\lim}\frac{1}{2n+1}\rightarrow0$，我们可以把$\underset{\alpha=n+1}{\overset{2n+1}{\Sigma}}\frac{1}{\alpha}$近似视作$\underset{\alpha=1}{\overset{2n}{\Sigma}}\frac{1}{\alpha}-\underset{\alpha=1}{\overset{n}{\Sigma}}\frac{1}{\alpha}$的结果，最终的结果与上述总数为$2n$的情况相同。



而对于$2n+1$允许开启$n+1$个抽屉的情况而言，可以推导假设$x（n+1\lt x \leqslant2n+1）$为最大环长度时：

设事件A为最大环长度不超过$n+1$，设事件B为最大环长度超过$n+1$，X表示最大环长度。

在这种情况下，最大环长度超过$n+1$时，图中剩余点的个数少于$n$，因此最大环长度为$n+2$与最大环长度大于$n+1$的其他情况仍然属于互斥事件。

对于最大环而言，随机选取$x$个点，他们组成一个环的所有可能为$C_{2n+1}^{x}\cdot(x-1)!$种。
...
$P(B)=\frac{1}{n+2}+\frac{1}{n+3}+\cdots+\frac{1}{2n+1}=\underset{\alpha=n+2}{\overset{2n+1}{\Sigma}}\frac{1}{\alpha}$

我们可以利用$\underset{\alpha=1}{\overset{2n+2}{\Sigma}}\frac{1}{\alpha}-\underset{\alpha=1}{\overset{n+1}{\Sigma}}\frac{1}{\alpha}-\frac{1}{2n+2}$得到$\underset{\alpha=n+2}{\overset{2n+1}{\Sigma}}\frac{1}{\alpha}$，由于$\underset{n\rightarrow \infty}{\lim}\frac{1}{2n+2}\rightarrow0$，我们可以把$\underset{\alpha=n+2}{\overset{2n+1}{\Sigma}}\frac{1}{\alpha}$近似视作$\underset{\alpha=1}{\overset{2n+2}{\Sigma}}\frac{1}{\alpha}-\underset{\alpha=1}{\overset{n+1}{\Sigma}}\frac{1}{\alpha}$的结果。

根据调和级数公式：$\underset{\alpha=1}{\overset{n}{\Sigma}}\frac{1}{\alpha}=ln(n)+\gamma$

$P(B)=\underset{\alpha=n+2}{\overset{2n+1}{\Sigma}}\frac{1}{\alpha}=ln(2n+2)+\gamma-(ln(n+1)+\gamma)=ln(2n+2)-ln(n+1)=ln(2)$

由$P(A)=1-P(B)$可以得到：

$P(A)=1-P(B)=1-\underset{\alpha=n+2}{\overset{2n+1}{\Sigma}}\frac{1}{\alpha}=1-ln(2)\approx0.306853$

因此采用上述的策略当$n$趋于无穷时可以有约30%的概率能够使得所有囚犯获得特赦。



### 探究分组对概率的影响

总数量从100变化到$n$趋于无穷大的情况时概率的变化并不大，因此在此策略下可以探究通过分组并改变规则来改变对应的概率，100个囚犯进行一次活动和200个囚犯平均分为两组分别活动的对比。

我们首先可以按照上述的方法计算一下两百人能够获得特赦的概率，约为$0.30934656951817578470$，以下近似为$30.93\%$，对于两个100人的组，每个组成功的概率近似为$31.18\%$

两组全部完成特赦，则特赦的概率为$31.18\%\times31.18\%\approx9.72\%$，这种规则下明显降低了特赦的概率。

两组结果一致特赦，则特赦的概率为$31.18\%\times31.18\%+68.82\%\times68.82\%\approx57.08\%$，通过这种方式提高了特赦的概率。

程序验证的结果如下：

| 序号 | 次数 | 人数 | 总概率 | 全完成概率 | 同或概率 |
| ---- | ---- | ---- | ------ | ---------- | -------- |
| 1    | 5000 | 5000 | 30.48% | 9.30%      | 57.60%   |
| 2    | 5000 | 5000 | 31.60% | 9.92%      | 56.84%   |
| 3    | 5000 | 5000 | 30.16% | 9.30%      | 57.60%   |
| 平均 | /    | /    | 30.75% | 9.51%      | 57.35%   |

对应report.txt中Type: 3的数据部分。



## 简要总结

通过这一次大作业，我体会到了一些看上去概率很低的事件也会有概率相对较高的解决方案，遇到问题时应该冷静分析背后的关系。在其他条件相同的情况下，采用不同的规则也能很大程度上改变原有的概率。
...
// 当替换为101和201后 输出为:
// 0.69065343048182421530
// 0.30934656951817578470
...
public class Main {
	public static void main(String []args) {
		...
		for(int cursor = 1; cursor < 101; cursor++) {
			bigDecimal = bigDecimal.divide(new BigDecimal(2), 100, RoundingMode.HALF_DOWN);
			resultBigDecimal = resultBigDecimal.add(bigDecimal.multiply(new BigDecimal(cursor)));
		}
...
// Output:
// 1.9999999999999999999999999999195361876674567958480 \
// 036863411558045733329436188796535134315490722656250
```
	...
	private class DataCreater implements Runnable {
		...
		public void run() {
			...
			for(int cursor = this.size - 1; cursor >= 0; cursor--) {
				numberArrayList.add(cursor); // Reverse input.
			}
