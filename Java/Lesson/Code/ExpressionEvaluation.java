...
//        }
    ...
    public static void replace(String inputString,Vector<String> expression){
        ...
        for(int i=0;i<inputString.length();i++){
            for(int j=0;j<opertor.length;j++){
                if(inputString.charAt(i)==opertor[j]){
                    end=i;
                    if(end>=start+1){
                        expression.add(inputString.substring(start,end));
                    }
                    expression.add(String.valueOf(opertor[j]));
                    start=i+1;
                    break;
                }
            }
    ...
    public static void infixToPolish(Vector<String> expression){
        ...
        for(int i=expression.size()-1;i>=0;i--){
            ...
            for(int j=0;j<opertor.length;j++){
                if(expression.get(i).equals(String.valueOf(opertor[j]))){
                    operatorBool=true;
